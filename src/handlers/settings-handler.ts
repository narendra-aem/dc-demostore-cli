...
export class SettingsHandler extends ResourceHandler {
  ...
  async import(context: ImportContext) {
    ...
    settings = _.isEqualWith(
      _.pick(hub.settings, Object.keys(settings)),
      settings,
      (a: any, b: any) => {
        return Array.isArray(a) && Array.isArray(b)
          ? _.isEqual(_.sortBy(a), _.sortBy(b))
          : undefined;
      },
    )
      ? {}
    ...
    workflowStates = _.reject(workflowStates, (fws) =>
      _.includes(
        _.map(_.uniqBy(hubWorkflowStates, "label"), "label"),
        fws.label,
      ),
    );
    ...
    if (!_.isEmpty(settings) || !_.isEmpty(workflowStates)) {
      ...
      await new CLIJob(
        `npx @amplience/dc-cli@latest settings import ${settingsJSONFile}`,
      ).exec();
    } else {
