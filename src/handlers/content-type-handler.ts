  ...
  for (const [filename, contentType] of Object.entries(importedContentTypes)) {
    if (contentType.contentTypeUri) {
      const otherFilenames: string[] =
        uriToFilenameMap.get(contentType.contentTypeUri) || [];
      if (filename) {
        uriToFilenameMap.set(contentType.contentTypeUri, [
          ...otherFilenames,
          filename,
        ]);
      }
  ...
  if (uniqueDuplicateUris.length > 0) {
    throw new Error(
      `Content Types must have unique uri values. Duplicate values found:-\n${uniqueDuplicateUris
        .map(
          ([uri, filenames]) =>
            `  uri: '${uri}' in files: [${filenames.map((f) => `'${f}'`).join(", ")}]`,
        )
        .join("\n")}`,
    );
  }
...
const installTypes = async (
  context: ImportContext,
  types: ContentTypeWithRepositoryAssignments[],
) => {
  const activeContentTypes: ContentType[] = await paginator(
    context.hub.related.contentTypes.list,
    { status: "ACTIVE" },
  );
  const archivedContentTypes: ContentType[] = await paginator(
    context.hub.related.contentTypes.list,
    { status: "ARCHIVED" },
  );
  ...
  await Promise.all(
    types.map(async (fileContentType) => {
      let stored = _.find(
        storedContentTypes,
        (ct) => ct.contentTypeUri === fileContentType.contentTypeUri,
      ...
      if (stored) {
        if (stored.status === "ARCHIVED") {
          ...
          archiveCount++;
          logUpdate(
            `${prompts.unarchive} content type [ ${chalk.gray(fileContentType.contentTypeUri)} ]`,
          );
        }
        ...
        updateCount++;
        logUpdate(
          `${prompts.update} content type [ ${chalk.gray(fileContentType.contentTypeUri)} ]`,
        );
      } else {
        stored = (await context.hub.related.contentTypes.register(
          fileContentType,
        )) as ContentTypeWithRepositoryAssignments;
        createCount++;
        logUpdate(
          `${prompts.create} content type [ ${chalk.gray(fileContentType.contentTypeUri)} ]`,
        );
      }
    }),
  );

  let repos: ContentRepository[] = await paginator(
    context.hub.related.contentRepositories.list,
  );
  let activeTypes: ContentType[] = await paginator(
    context.hub.related.contentTypes.list,
    { status: "ACTIVE" },
  );
  ...
  await Promise.all(
    repos.map(async (repo) => {
      await Promise.all(
        types.map(async (fileContentType) => {
          fileContentType.repositories = fileContentType.repositories || [
            "sitestructure",
          ];
          let activeType = _.find(
            activeTypes,
            (type) => type.contentTypeUri === fileContentType.contentTypeUri,
          );
          if (
            activeType &&
            ...
            assignedCount++;
            logUpdate(
              `${prompts.assign} content type [ ${chalk.grey(fileContentType.contentTypeUri)} ]`,
            );
  ...
  await Promise.all(
    _.filter(activeTypes, (t) =>
      _.includes(_.map(types, "contentTypeUri"), t.contentTypeUri),
    ).map(async (type) => {
      synchronizedCount++;
      await type.related.contentTypeSchema.update();
      logUpdate(
        `${prompts.sync} content type [ ${chalk.gray(type.contentTypeUri)} ]`,
      );
    }),
  );
};
...
export class ContentTypeHandler extends CleanableResourceHandler {
  ...
  async import(context: ImportContext): Promise<any> {
    ...
    logComplete(
      `${this.getDescription()}: [ ${chalk.green(archiveCount)} unarchived ] [ ${chalk.green(updateCount)} updated ] [ ${chalk.green(createCount)} created ] [ ${chalk.green(synchronizedCount)} synced ]`,
    );
  }
  ...
  async cleanup(context: CleanupContext): Promise<any> {
    ...
    await Promise.all(
      repos.map(async (repo) => {
        ...
        if (repoTypes) {
          await Promise.all(
            repoTypes.map(async (type) => {
              unassignedCount++;
              logUpdate(
                `${prompts.unassign} content type [ ${chalk.grey(type.contentTypeUri)} ]`,
              );
    ...
    logComplete(
      `${chalk.cyan(`📦  repositories`)}: [ ${chalk.red(unassignedCount)} content types unassigned ]`,
    );
