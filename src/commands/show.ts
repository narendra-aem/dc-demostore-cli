...
export const handler = contextHandler(
  async (context: AmplienceContext): Promise<void> => {
    ...
    let contentTypeSchemas = await paginator(
    ...
    let count: any = _.zipObject(
      _.map(repositories, (r) => r.name || ""),
      await Promise.all(
        repositories.map(async (repo) => {
          logUpdate(`reading repository ${repo.name}...`);
          let start = new Date().valueOf();
          let x = (
            await paginator(repo.related.contentItems.list, {
              status: "ACTIVE",
            })
          ).length;
          logUpdate(
            `repo ${repo.name} read [ ${x} ] content items in ${new Date().valueOf() - start} ms`,
          );
          return x;
        }),
      ),
    ...
    count.extensions = (
      await paginator(context.hub.related.extensions.list, { status: "ACTIVE" })
    ).length;

    logUpdate(`reading webhooks...`);
    count.webhooks = (
      await paginator(context.hub.related.webhooks.list, { status: "ACTIVE" })
    ).length;

    logUpdate(`reading events...`);
    count.events = (
      await paginator(context.hub.related.events.list, { status: "ACTIVE" })
    ).length;
