...
export const fetchRemoteTemplateFiles = async (): Promise<void> => {
  ...
  logger.info(
    `downloading latest automation files to ${chalk.blue(AUTOMATION_DIR_PATH)}...`,
  );
  ...
  return new Promise((resolve, reject) => {
    response.data.on("end", async () => {
      ...
      fs.moveSync(
        `${CONFIG_PATH}/dc-demostore-automation-main`,
        AUTOMATION_DIR_PATH,
      );
...
export const setupAutomationTemplateFiles = async (
  context: ImportContext | CleanupContext,
) => {
...
export const processAutomationTemplateFiles = async (
  context: ImportContext | CleanupContext,
) => {
  ...
  fs.writeFileSync(
    `${context.tempDir}/content_mapping.json`,
