...
export class DAMService {
  ...
  async getBucketsList(): Promise<any> {
    const bucketsList = await this.client.fetchResource(
      `/buckets?_=${Date.now}`,
    );
    return bucketsList;
  }
  ...
  async updateBucket(bucketId: string, data: any): Promise<string> {
    const updatedBucketId = await this.client.updateResource(
      `/buckets/${bucketId}`,
      bucketId,
      data,
    );
  ...
  async getAssetsListForBucket(bucketName: any): Promise<any> {
    ...
    const assetsList = await this.client.fetchPaginatedResourcesList(
      `/assets?filter=bucketID:${bucket.id}`,
    );
    return assetsList;
  }
  ...
  async getEndpoints(): Promise<any> {
    const endpoints =
      await this.client.fetchPaginatedResourcesList(`/endpoints`);
    return endpoints;
  }
  ...
  async getAssetsList(bucketId: any): Promise<any> {
    const assetsList = await this.client.fetchPaginatedResourcesList(
      `/assets?filter=bucketID:${bucketId}`,
    );
    return assetsList;
  }
  ...
  async publishAssets(data: any): Promise<any> {
    const assetsList = await this.client.publishResources(
      `/assets/publish`,
      data,
    );
    return assetsList;
  ...
  async getFoldersList(bucketId: any): Promise<any> {
    const foldersList = await this.client.fetchResource(
      `/folders?filter=bucketID:${bucketId}`,
    );
    ...
    const addChildrenToList = (childrenList: any[], node: any) => {
      ...
      if (node.children) {
        node.children.forEach((item: any) =>
          addChildrenToList(childrenList, item),
        );
