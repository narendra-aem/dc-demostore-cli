...
export const builder = (yargs: Argv): void => {
  yargs
    .option("dstHubId", {
      type: "string",
      describe:
        "Destination hub ID. If not specified, it will be the same as the source.",
    })

    .option("dstClientId", {
      type: "string",
      describe:
        "Destination account's client ID. If not specified, it will be the same as the source.",
    })

    .option("dstSecret", {
      type: "string",
      describe:
        "Destination account's secret. Must be used alongside dstClientId.",
    });
};
...
const writeConfigFile = (
  ...
  try {
    ...
    throw new Error(
      `Unable to write config file "${configFile}". Reason: ${err}`,
    );
  }
};
...
export const handler = (
  argv: Arguments<ConfigurationParameters & ConfigArgument>,
): void => {
  ...
  if (isEqual(storedConfig, newConfig)) {
    console.log(
      "Config file up-to-date.  Please use `--help` for command usage.",
    );
    return;
  }
