...
export class EventHandler extends ResourceHandler {
  ...
  async cleanup(context: CleanupContext): Promise<any> {
    let events: Event[] = await paginator(context.hub.related.events.list, {
      status: "ACTIVE",
    });
    await Promise.all(
      events.map(async (event) => {
        ...
        let publishedEditions = _.filter(
          editions,
          (e) => e.publishingStatus === PublishingStatus.PUBLISHED,
        );
          ...
          editions.map(async (edition) => {
            if (edition.publishingStatus !== PublishingStatus.PUBLISHED) {
              if (edition.publishingStatus === PublishingStatus.SCHEDULED) {
                logger.info(
                  `${prompts.unschedule} edition [ ${chalk.gray(edition.name)} ]`,
                );
                await edition.related.unschedule();
              }
              logger.info(
                `${prompts.delete} edition [ ${chalk.gray(edition.name)} ]`,
              );
