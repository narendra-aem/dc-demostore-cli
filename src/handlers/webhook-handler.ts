...
export class WebhookHandler extends CleanableResourceHandler {
    ...
    constructor() {
     ...
     async import(context: ImportContext): Promise<any> {
        ...
        await Promise.all(webhooks.map(async (wh: Webhook) => {
            try {
                ...
                logger.error(`${prompts.error} importing webhook [ ${wh.label} ]: ${error.message}`)
            }
        }))
    ...
    async cleanup(context: CleanupContext): Promise<any> {
        ...
        try {
            let deleteCount = 0
            let webhooks: Webhook[] = await paginator(context.hub.related.webhooks.list)
