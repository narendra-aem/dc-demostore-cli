...
export class SearchIndexHandler extends ResourceHandler {
  ...
  async import(context: ImportContext): Promise<any> {
    ...
    for (const { indexDetails, settings } of indexFixtures) {
      try {
        logUpdate(
          `apply index settings: ${chalk.cyanBright(indexDetails.name)}`,
        );
        const index = client.initIndex(indexDetails.name);
        index.setSettings(settings);
      } catch (error) {
        logger.error(
          `${prompts.error} applying index settings [ ${indexDetails.name} ]: ${error.message}`,
        );
      }
    }

    logComplete(
      `${this.getDescription()}: [ ${chalk.green(indexFixtures.length)} updated ]`,
    );
  ...
  async cleanup(context: CleanupContext) {
    ...
    logComplete(
      `${this.getDescription()}: [ ${chalk.red(indexFixtures.length)} cleared ]`,
    );
  }
}
