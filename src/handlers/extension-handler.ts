...
export class ExtensionHandler extends ResourceHandler implements Cleanable {
      ...
      extensions.map(async (ext: Extension) => {
        try {
          ...
          if (error.message.indexOf("EXTENSION_NAME_DUPLICATE")) {
            logger.error(
              `${prompts.error} importing extension [ ${ext.name} ]: duplicate name`,
            );
          } else {
            logger.error(
              `${prompts.error} importing extension [ ${ext.name} ]: ${error.message}`,
            );
          }
        }
      }),
  ...
  async cleanup(context: CleanupContext): Promise<any> {
    ...
    try {
      let deleteCount = 0;
      let extensions: Extension[] = await paginator(
        context.hub.related.extensions.list,
      );
      await Promise.all(
