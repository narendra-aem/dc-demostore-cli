...
export class DAMClient {
  ...
  async init(argv: ConfigurationParameters) {
    try {
      ...
      throw new Error(
        `error logging in to content hub, check your credentials`,
      );
    }
  }
  ...
  async fetchPaginatedResourcesList(endpointUri: string): Promise<any[]> {
    ...
    if (this.accessToken) {
      // Retrieve resources using REST API
      const additionalParamsSeparator =
        endpointUri.indexOf("?") > 0 ? "&" : "?";
      const response = await fetch(
        `${this.apiUrl}${endpointUri}${additionalParamsSeparator}n=${this.PAGE_SIZE}`,
        {
          method: "GET",
          headers: { "X-Amp-Auth": this.accessToken },
        },
