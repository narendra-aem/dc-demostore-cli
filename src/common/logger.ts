...
const decolorizeString = (str: string) =>
  str.replace(
...
export const logUpdate = (message: string, logToFile: boolean = true) => {
  ...
  if (logger.level !== "debug") {
    ...
    process.stdout.write(
      `\r\r${chalk.bgWhite.black.bold("exec")}  ${message}${" ".repeat(numSpaces)}`,
    );
  }
};
...
export const logRunEnd = (context: LoggableContext) => {
  ...
  logger.info(
    `logs and temp files stored in ${chalk.blueBright(context.tempDir)}`,
  );
