...
export const builder = (yargs: Argv): Argv => {
  return amplienceBuilder(yargs)
    ...
    .command(
      ...
      "Import extensions",
      {},
      importHandler(new ExtensionHandler()),
    )
    .command(
      "webhooks",
      "Import webhooks",
      {},
      importHandler(new WebhookHandler()),
    )
    .command(
      "settings",
      "Import settings",
      {},
      importHandler(new SettingsHandler()),
    )
    .command(
      "types",
      "Import content types/schemas",
      {},
      importHandler(new ContentTypeSchemaHandler()),
    )
    .command(
      "search-indexes",
      "Import search indexes",
      {},
      importHandler(new SearchIndexHandler()),
    );
};

const importHandler =
  (handler: Importable) =>
  async (context: ImportContext): Promise<void> => {
...
export const handler = contextHandler(
  async (context: ImportContext): Promise<void> => {
    logger.info(
      `${chalk.green(command)}: ${desc} started at ${chalk.magentaBright(context.startTime)}`,
    );
